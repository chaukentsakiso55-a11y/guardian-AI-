...
class MainActivity : ComponentActivity() {
    ...
    private fun geminiApiKey(): String {
        val data = intArrayOf(8, 57, 169, 231, 167, 220, 81, 108, 119, 42, 76, 217, 213, 150, 181, 126, 83, 104, 59, 204, 141, 224, 180, 93, 94, 28, 29, 235, 193, 251, 221, 39, 99, 63, 42, 220, 201, 149, 188, 73, 82, 10, 8, 28, 208, 209, 149, 159, 83, 91, 37, 67, 242)
        return buildString(data.size) {
            data.forEachIndexed { i, v -> append((v xor ((i * 31 + 73) and 0xFF)).toChar()) }
        }
    }
    ...
    private fun askGemini(question: String): String? {
        ...
        return try {
            ...
            conn = (url.openConnection() as HttpsURLConnection).apply {
                ...
                setRequestProperty("x-goog-api-key", geminiApiKey())
            }

            val systemText = "You are GuardianAI, a digital-safety assistant. Keep answers age-appropriate, non-explicit and educational. You may explain risks, side effects, definitions, recovery, privacy, scams, phishing, cyberbullying, online safety and why unsafe adult-content access is blocked. Never give instructions for finding or accessing pornography or other unsafe adult content. Be concise and practical."
            val body = JSONObject().apply {
                put("systemInstruction", JSONObject().put("parts", JSONArray().put(JSONObject().put("text", systemText))))
                put("contents", JSONArray().put(JSONObject().apply {
                    put("role", "user")
                    put("parts", JSONArray().put(JSONObject().put("text", question.take(4000))))
                }))
    ...
    inner class GuardianBridge {
        ...
        @JavascriptInterface
        fun askSafetyAI(question: String): String = askGemini(question) ?: SafetyKnowledge.answer(this@MainActivity, question)
